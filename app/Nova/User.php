		...
		return [
			...
			ResourceNavigationTab::make([
				...
				'fields' => [
					...
					HasMany::make(__('bonuses'), 'bonuses', Bonuse::class)->canSee(fn ($request) =>  $request->user()->hasRole('Superadmin')),
					HasMany::make(__('gifts'), 'gifts', Gift::class)->canSee(fn ($request) =>  $request->user()->hasRole('Superadmin')),
	...
	public function actions(Request $request): array
	{
		return [
			(new NotifyAction())
				...
				->canSee(fn ($request) => $request->user()->hasRole('Admin') || $request->user()->hasRole('Superadmin'))->showOnTableRow(),
