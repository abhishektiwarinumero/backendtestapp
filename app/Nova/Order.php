...
class Order extends Resource
{
	...
	public function actions(Request $request): array
	{
		return [
			...
			(new MarkOrderAsPaid)
				->showOnTableRow()
				->canRun(fn ($request) => $request->user()->hasRole('Admin')|| $request->user()->hasRole('Superadmin'))
				->canSee(fn ($request) => $request->user()->hasRole('Admin')|| $request->user()->hasRole('Superadmin')),
			(new CloseOrder)
				->showOnTableRow()
				->canSee(fn ($request) => !$request->user()->hasRole('Member') && ($this->status != 'paid' && $this->status != 'completed'))
				->canRun(fn ($request) => !$request->user()->hasRole('Member') && ($this->status != 'paid' && $this->status != 'completed')),
			(new ReleaseOrder)
				->showOnTableRow()
				->canSee(fn ($request) => !$request->user()->hasRole('Member'))
